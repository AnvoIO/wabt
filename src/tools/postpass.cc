 ...
 * Copyright 2018-2019 block.one and its contributors
...
// CDT WASM post-processor: strips zero-initialized data segments (BSS),
// coalesces remaining segments, and fixes the heap pointer global.
// Invoked automatically by cdt-ld after wasm-ld linking.
...
R"(  Read a file in the WebAssembly binary format, strip bss or any data
  segment that is only initialized to zeros, and other post processing.
...
static void ParseOptions(int argc, char** argv) {
  ...
  parser.AddOption(
      'o', "output", "FILENAME",
      "Output file for the generated wasm file, by default use stdout",
      [](const char* argument) {
        s_outfile = argument;
...
uint32_t GetHeapPtr(Module& mod, const std::vector<uint8_t>& buff) {
  size_t offset =
      mod.GetGlobal(Var(Index(1), Location()))->init_expr.begin()->loc.offset;
  uint32_t heap_ptr;
  ReadS32Leb128(buff.data() + offset + 4, buff.data() + offset + 9,
                &heap_ptr);
  return heap_ptr;
}
...
uint32_t GetStackPtr(Module& mod, const std::vector<uint8_t>& buff) {
  size_t offset =
      mod.GetGlobal(Var(Index(0), Location()))->init_expr.begin()->loc.offset;
  uint32_t stack_ptr;
  ReadS32Leb128(buff.data() + offset + 4, buff.data() + offset + 9,
                &stack_ptr);
  return stack_ptr;
...
std::vector<DataSegment*> StripZeroedData(std::vector<DataSegment*>&& ds,
                                          size_t& fix_bytes) {
...
inline std::vector<uint8_t> FillFromSegments(
    const std::vector<DataSegment*>& segments) {
  std::vector<uint8_t> memory;
  auto last_segment = segments.back();
  ConstExpr* ce =
      reinterpret_cast<ConstExpr*>(&(last_segment->offset.front()));
  memory.resize(ce->const_.u32() + last_segment->data.size());

  for (auto ds : segments) {
    auto offset =
        reinterpret_cast<ConstExpr*>(&(ds->offset.front()))->const_.u32();
...
inline DataSegment* CreateSegment(uint32_t offset, uint8_t* start,
                                  std::size_t size) {
...
inline std::vector<DataSegment*> CreateSegments(
    std::vector<uint8_t> memory) {
  ...
  for (std::size_t i = f; i < memory.size(); ++i) {
    ...
    if (zero_span > 8 || (size > 1024)) {
      segments.push_back(
          CreateSegment(last_offset, &memory[last_offset], size));
      last_offset = i;
      zero_span = 0;
      i += size;
      continue;
  ...
  segments.push_back(CreateSegment(last_offset, &memory[last_offset],
                                   memory.size() - last_offset));

  return segments;
}

void AddHeapPointerData(Module& mod, size_t fixup,
                        const std::vector<uint8_t>& buff, DataSegment& ds) {
  ...
  ds.data = std::vector<uint8_t>{dat[0], dat[1], dat[2], dat[3]};
    ...
    if (Succeeded(result)) {
      ...
      if (!module.data_segments.empty()) {
        ...
        if (pre_memory != FillFromSegments(segments)) {
          std::cerr << "Fractured Memory Failed, not applying optimizations"
                    << std::endl;
          module.data_segments =
              StripZeroedData(std::move(module.data_segments), fixup);
        } else {
          module.data_segments =
              StripZeroedData(std::move(segments), fixup);
        }
      }
